...
(function ($) {
    ...
    $.fn.chart = function (url, form, callback) {
        $(this).unbind('click').click(function () {
            callback();
            var params = '';
            if (form) {
    ...
    $.parseParams = function (arr) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (!obj[key]) {
                ...
                var oldVal = obj[key];
                obj[key] = oldVal.split(",").concat(value);
            }
        }
        var str = "";
        for (var j = 0; j < newArr.length; j++) {
            var keyword = newArr[j];
            if (j != 0) {
                str += "&";
            }
            var val = obj[keyword];
            if(val instanceof Array){
                str += keyword + "=" + val.join(",");
            }else{
                str += keyword + "=" + val;
            }
        }
